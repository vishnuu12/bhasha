    ...
    <DialogContent className="max-h-[85dvh] overflow-y-auto sm:max-w-lg">
      ...
      <FieldGroup>
        <Field><FieldLabel id="engine-label">Speech engine</FieldLabel>
          <ToggleGroup aria-labelledby="engine-label" value={[voice.engine]} onValueChange={values => { if (values[0]) { voice.stopAudio(); voice.setEngine(values[0] as SpeechEngine) } }} variant="outline" disabled={locked}>
          ...
          <FieldDescription>Auto selects input and output independently. Cloud access is confirmed on use; if transcription fails, switch engines and re-record. Text chat always remains available.</FieldDescription>
        </Field>
        <Field><FieldLabel id="speed-label">Speaking speed</FieldLabel><ToggleGroup aria-labelledby="speed-label" value={[String(voice.speed)]} onValueChange={values => { if (values[0]) { voice.stopAudio(); voice.setSpeed(Number(values[0])) } }} variant="outline" disabled={locked}><ToggleGroupItem value="0.85">Relaxed</ToggleGroupItem><ToggleGroupItem value="1">Natural</ToggleGroupItem><ToggleGroupItem value="1.15">A little faster</ToggleGroupItem></ToggleGroup></Field>
        <Separator />
